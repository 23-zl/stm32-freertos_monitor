...
volatile uint32_t SystemParam = 0;
struct {
    float value1;
    float value2;
    float value3;
...
int main(void)
{
  ...
  Queue_UART_TXHandle = osMessageQueueNew (16, sizeof(uint16_t), &Queue_UART_TX_attributes);
  ...
  Task_ADCHandle = osThreadNew(StartTaskADC, NULL, &Task_ADC_attributes);
